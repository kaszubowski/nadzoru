...
function ScadaView:build_gui()
    ...
    --connect/disconnect
    self.img_act_connect    = gtk.Image.new_from_file( './images/icons/connect.png' )
    self.img_act_disconnect = gtk.Image.new_from_file( './images/icons/disconnect.png' )
        ...
        if event_name then
            ...
            for k, enviroment in ipairs( self.run.components_env ) do
                if enviroment.onupdate then
                    enviroment.onupdate( enviroment.component, event_name, self.run.simulators )
                end
            end
