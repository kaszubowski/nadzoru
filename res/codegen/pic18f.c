...
void delay_ms(unsigned long int ms){
    unsigned long int v = (_XTAL_FREQ_MHZ/4)*ms;
    if(v>1000){
        delay1mtcy(v/1000);
        v %= 1000;
    }
...
void delay_us({{int}} us){
    unsigned long int v = (_XTAL_FREQ_MHZ/4)*us;
    int i;
    if(v>1000){
        delay1ktcy(v/1000);
        v %= 1000;
...
void Intr(void) __interrupt 1
{
    ...
    breg=PORTB;
    PORTB=breg;
    //INTCONbits.RBIF=0;     // Clear PORTB4...7 interrupt flag (bit 0)
